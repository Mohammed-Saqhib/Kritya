...
// Demo mode flag - THIS IS ALWAYS TRUE FOR PORTFOLIO DEMO
const DEMO_MODE = true;

// Security notice for developers
console.warn("⚠️ PORTFOLIO DEMONSTRATION WEBSITE ⚠️");
console.warn("This is NOT a real e-commerce website.");
console.warn("No real transactions, payments, or data collection occurs.");
console.warn("All functionality is simulated for portfolio demonstration purposes.");

// Prevent any accidental real data submission
if (DEMO_MODE) {
    // Override any external API calls
    window.fetch = function(...args) {
        console.warn("DEMO MODE: Fetch request blocked:", args);
        return Promise.reject(new Error("Demo mode: External requests blocked"));
    };
    
    // Show demo warning on page load
    setTimeout(() => {
        if (!localStorage.getItem('demoWarningShown')) {
            alert("⚠️ PORTFOLIO DEMO NOTICE ⚠️\n\nThis is a demonstration website for a developer portfolio.\n\nDo NOT enter real personal information.\nNo actual payments or transactions will occur.\n\nThis showcases web development skills only.");
...
function initializeDemoMode() {
    if (DEMO_MODE) {
        // Add demo banner dismiss functionality
        const dismissBtn = document.getElementById('dismissDemo');
        if (dismissBtn) {
        ...
        // Override WhatsApp functionality to show demo message
        const whatsappBtn = document.getElementById('whatsappBtn');
        if (whatsappBtn) {
            whatsappBtn.addEventListener('click', function() {
                alert('DEMO MODE: This would normally redirect to WhatsApp for order confirmation. No actual message will be sent.');
            });
        }
    }
}
...
function handleSignIn(e) {
    ...
    // Demo mode warning
    if (DEMO_MODE) {
        console.log("DEMO MODE: Simulating sign-in process");
    }
    ...
    if (!phone || !password) {
        showAuthError('Please fill in all fields (Demo: Use any values)');
        return;
    }

    // In demo mode, always accept demo credentials
    if (DEMO_MODE) {
        currentUser = {
            id: 'demo-user-' + Date.now(),
            name: 'Demo Portfolio User',
            phone: phone,
            upi: 'demo@portfolio',
            password: password,
        ...
        closeAuthModal();
        saveUserToStorage();
        showSuccess('Demo sign-in successful! (No real account created)');
        return;
    }

    // This code would only run if DEMO_MODE was false
...
function handleSignUp(e) {
    ...
    if (!name || !phone || !upi || !password || !confirmPassword) {
        showAuthError('Please fill in all fields (Demo: Use any dummy values)');
        return;
    }

    // In demo mode, always create demo account with minimal validation
    if (DEMO_MODE) {
        if (password !== confirmPassword) {
            showAuthError('Passwords do not match (Demo: Use any matching values)');
            return;
        }

        const newUser = {
            id: 'demo-user-' + Date.now(),
            name: name || 'Demo Portfolio User',
            phone: phone || '1234567890',
            upi: upi || 'demo@portfolio',
            password: password,
            createdAt: new Date().toISOString()
        };
        ...
        closeAuthModal();
        saveUserToStorage();
        showSuccess('Demo account created! (No real account created - portfolio demo only)');
        return;
    }
...
function openPaymentModal() {
    if (!currentUser) {
        showError('Please sign in to proceed with demo checkout');
        openAuthModal();
        return;
    }

    if (cart.length === 0) {
        showError('Your demo cart is empty');
        return;
    }

    // Show demo warning for payment
    if (DEMO_MODE) {
        console.log("DEMO MODE: Opening simulated payment modal");
    }
...
function redirectToWhatsApp() {
    ...
    // DEMO MODE: Show alert instead of real WhatsApp redirect
    if (DEMO_MODE) {
        alert(`DEMO MODE: This would normally redirect to WhatsApp with order details.\n\nOrder Summary:\nOrder ID: ${order.id}\nTotal: ₹${order.total}\n\nNo actual WhatsApp message will be sent.`);
        closeSuccessModal();
        return;
    }
    
    // This code would only run if DEMO_MODE was false (which it never is in this demo)
